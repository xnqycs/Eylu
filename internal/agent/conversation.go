...
func NewConversation() *Conversation {
	conversation := &Conversation{sessionID: uuid.NewString(), closed: make(map[string][]protocol.Turn), ledger: contextledger.New(nil)}
...
func (c *Conversation) rebuildLedger(runtime Runtime) {
	c.ledger.Reset()
	c.ledger.AddText("system", contextledger.CategorySystemPrompt, "eylu", SystemPrompt, true)
