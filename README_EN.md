...
- Items 1 and 2 of §8.3 (in-line byte intervals, the conservative degradation marker) are therefore **real work that is not implemented yet**: they change the coverage judgement itself, extending "line ranges" to "line ranges plus byte intervals within a line", which is exactly where red line 1's body correctness is easiest to break. The reproduction case is kept so the implementation has something to satisfy.
