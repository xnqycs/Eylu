...
- 重复读取去重收益实测（真实读取、逐形状）：超长单行 40,001→~173 token（+39,828），而小正文去重是**净亏**（引用长度基本固定 ~173 token，比正文大）；据此记录"去重总是省 token"不成立。同时记录：计划 A11 的前提（超长单行拿不到去重收益）**未能复现**，故 §8.3 第 1/2 条未实施，待能复现该形状再动。
