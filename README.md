...
- **写入次数**：一个批次里 N 个副作用调用的**意图合并为一次 append**（写在批次内任何调用开始之前，因此"意图先于副作用"的保证不变）；completion 仍然每个调用各写一次——合并 completion 能再省 N 次写入，但会把"已经发生的事"的记录推迟到进程可能已经死掉之后，对没有文件可查的副作用而言，那等于把已知结果变成未知，所以这里不省。实测：N=3 的批次从 2N=6 次 append 降到 4 次（1 次意图 + 3 次 completion），并有断言固定这个数字。单次 append 的实测成本约 **1.0 ms**（复用索引，见大日志基准），所以 3 个调用的批次把这笔开销从约 6 ms 降到约 4 ms——**决定不合并 completion 的理由是正确性，不是这点开销**：省下的是噪声级别的时间，代价却是把「已经发生的操作」的记录推迟到进程可能已死之后。
...
- **红线 1 的保护规则**（每条都有对应测试，见 `internal/context/byte_spans_test.go`）：
  1. 行内字节区间**永远不能**被当作行区间。一个只有部分字节的 canonical **不覆盖**该行的完整读取——否则就会把一个完整正文换成指向"并不包含它"的正文的引用。
  2. 只有当前片段**自己也是**同一行的部分片段、且它持有的字节被 canonical 的区间**完全包含**时，才允许用引用替换。
  3. 行内片段**永不**取代其他 canonical（它不持有任何完整行，没有资格当更大范围的权威），也永不改写已有引用。
  4. 引用文本会写明它代表**部分行**（`bytes=1:0-300,1:900-1000`），模型不会被误导为拿到整行。
  5. 文件哈希不同的字节是不同字节（沿用既有规则）。
- 其它情形（同一行更宽的片段、另一行的片段、另一版本的片段、无区间）全部**保持正文**，这些负向用例与正向用例一起构成验收条件。
